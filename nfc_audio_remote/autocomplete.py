...
class AutocompleteEntry(Entry):
        ...
        if self.var.get() == '':
            ...
            if wordsWithPayload:
                ...
                for w in wordsWithPayload:
                    self.listbox.insert(END,w["name"])
            else:
                if self.listboxUp:
                    self.listbox.destroy()
                    self.listboxUp = False
    ...
    def comparison(self):
        return [ w for w in self.autocompleteList if self.matchesFunction(self.var.get(), w["name"]) ]
...
if __name__ == '__main__':
    ...
    autocompleteListWithPayload = [
        { 'name': 'Dora Lyons', 'id': 7714 }
        , { 'name': 'Hannah Golden', 'id': 1 }
        , { 'name': 'Walker Burns', 'id': 2 }
        , { 'name': 'Allen Sullivan', 'id': 3 }
        , { 'name': 'Warren Sullivan', 'id': 4 }
    ]
    ...
    root = Tk()
    entry = AutocompleteEntry(autocompleteListWithPayload, root, listboxLength=6, width=32, matchesFunction=matches_internal)
